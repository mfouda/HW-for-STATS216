...
#We can effectively force j=0 for the j corresponding to Stanford by eliminating that column from the predictor matrix.
...
#Create a "schedule" function to put any team's schedule into a good format:
...
for (i in 1:k) {  
  ...
  #Compare predicted results to actual for both models and build a contingency table for each fold.
  #Store each contingency table into a list which can then be added later for the final table.
